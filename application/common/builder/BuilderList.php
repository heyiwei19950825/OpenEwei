...
class BuilderList extends Builder {
     ...
     * 加一个表格字段
     */
    public function keyListItem($field, $title, $sort = null, $edit = null,$toolbar=null) {
     ...
     * @author 心云间、凝听 <981248356@qq.com>
     */
    public function setListLimit($limit = 20) {
        if (is_null($limit)) {
     ...
     * @author 心云间、凝听 <981248356@qq.com>
     */
    public function setListLimits($limits = [20,50,100]) {
        if (is_null($limits)) {
            $limits = config('admin_page_size');
        }
        if ($limits === false) {
            $limits = $this->tableLimits;
        }
     ...
     * @return $this
     */
    public function openPage( $open = false){
        $this->openPage = $open;
        return $this;
    }
    ...
    public function fetch($template_name='listbuilder',$vars ='', $replace ='', $config = '') {
        ...
        if ($this->request->isAjax) {
            ...
            foreach ($this->tableColumns as $key => $val) {
                $table_column_fields[$key]['field']=$val['field'];
            ...
            $template_val = [
                ...
                'table_columns'       => json_encode($this->tableColumns),// 表格的列
//                'table_column_fields' => json_encode($table_column_fields),//表格列bootstrap-table
                ...
                'openPage'            => $this->openPage, //是否开启分页
                'table_limit'         => $this->tableLimit, //数据分页数量
