<?php /*a:1:{s:52:"D:\work\OpenEwei\application\admin\view\adv\pos.html";i:1542355213;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
    layui.config({
        ...
        table.on('tool(list)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'add') { //添加
                $('[lay-event="add"]').attr('lay-href', "<?php echo url('adv/appendAdv'); ?>&pos_id=" + data.id);
            } else if (layEvent === 'del') { //删除
                layer.confirm('确定删除吗？', function (index) {
                    obj.del(); //删除对应行（tr）的DOM结构，并更新缓存
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("<?php echo url('adv/delPos'); ?>", {id: data.id}, function (res) {
                        layer.msg('删除成功');
                    })
                });
                ...
                layer.open({
                    type: 2
                    , title: '编辑广告'
                    , content: "<?php echo url('adv/editPos'); ?>&id=" + data.id
                    , area: ['800px', '500px']
                    , btn: ['确定', '取消']
        ...
        var active = {
            del: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("<?php echo url('adv/delPos'); ?>", {id: ids}, function (res) {
                        layer.msg('删除成功');
                    });
                    table.reload('list');
                });
            ...
            add: function () {
                layer.open({
                    type: 2
                    , title: '添加广告'
                    , content: "<?php echo url('adv/editPos'); ?>"
                    , area: ['800px', '500px']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        var iframeWindow = window['layui-layer-iframe' + index]
        ...
        table.on('tool(data-list)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("<?php echo url('adv/delAdv'); ?>", {id: data.id}, function (res) {
                        layer.msg('删除成功');
                    })
                });
            } else if (layEvent === 'edit') { //编辑
                $('.edit').attr('lay-href', "<?php echo url('adv/appendAdv'); ?>&id=" + data.id);
            }
        });
    });
</script>
