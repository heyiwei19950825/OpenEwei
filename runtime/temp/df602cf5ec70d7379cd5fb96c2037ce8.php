<?php /*a:1:{s:58:"D:\work\OpenEwei\application\admin\view\user\user_log.html";i:1542355213;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
    layui.use(['table', 'jquery'], function () {
        ...
        table.render({
            elem: '#list',
            url: "<?php echo url('admin/user/userLog'); ?>", //数据接口
            title: '用户日志表',
            toolbar: 'true',
        ...
        var active = {
            del: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    $.post("<?php echo url('admin/user/delUserLog'); ?>", {id: ids}, function (res) {
                        if (res.code === 1) {
                            layer.close(index);
                            table.reload('list');
                            layer.msg(res.msg);
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (layEvent === 'del') {
                layer.confirm('真的删除行么', function (index) {
                    console.log(index);
                    $.post('<?php echo url("admin/user/delUserLog"); ?>', {id: [data.id]}, function (res) {
                        if (res.code === 1) {
                            obj.del();
                            layer.close(index);
                            layer.msg(res.msg);
