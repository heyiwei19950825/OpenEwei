<?php /*a:1:{s:71:"D:\work\yanglao_admin_3\application\admin\view\action\action_limit.html";i:1542355213;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
        ...
        var active = {
            del: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    $.post("<?php echo url('admin/action/delActionLimit'); ?>", {id: ids}, function (res) {
                        if (res.code === 1) {
                            layer.close(index);
                            table.reload('list');
                            layer.msg(res.msg);
            ...
            , add: function () {
                layer.open({
                    type: 2
                    , title: "添加用户行为限制"
                    , content: "<?php echo url('admin/action/actionLimitForm'); ?>"
                    , area: ['600px', '500px']
                    , maxmin: true
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
                            ...
                            $.post("<?php echo url('admin/action/actionLimitForm'); ?>", {data: data.field}, function (res) {
                                if (res.code === 1) {
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (layEvent === 'del') {
                layer.confirm('真的删除行么', function (index) {
                    $.post('<?php echo url("admin/action/delActionLimit"); ?>', {id: [data.id]}, function (res) {
                        if (res.code === 1) {
                            obj.del();
                            layer.close(index);
                            layer.msg(res.msg);
                ...
                layer.open({
                    type: 2
                    , title: '编辑用户行为限制'
                    , content: "<?php echo url('admin/action/actionLimitForm'); ?>?id=" + data.id
                    , maxmin: true
                    , area: ['600px', '500px']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
